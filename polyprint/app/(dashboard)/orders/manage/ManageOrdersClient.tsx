...
export default function ManageOrdersClient({ fullName, initialOrders = [] }: any) {
  const [loadingId, setLoadingId] = useState<string | null>(null);
  const supabase = createClient();

  const handleDownload = async (order: any) => {
    const fileUrl = order.order_items?.[0]?.file_url; // Corrected path from your logs
            ...
            {initialOrders.map((order: any) => (
              <tr key={order.id} className="hover:bg-blue-50/30">
                ...
                <td className="p-4 text-sm">
                  {order.order_items?.[0]?.service_type} (x{order.order_items?.[0]?.quantity})
                </td>
                <td className="p-4">
                ...
                <td className="p-4 text-right">
                    <button onClick={() => handleDownload(order)} className="mr-2 p-2 bg-gray-100 rounded-lg">💾</button>
                    <button 
                        onClick={() => handleStatusUpdate(order.id, order.status === 'approved' ? 'in_progress' : 'completed')}
                        disabled={loadingId === order.id}
                        className="px-4 py-2 bg-[#0D284A] text-white text-xs font-bold rounded-lg"
                    >
